...
class Login extends React.Component {
  ...
  sendSMS = () => {
    axios
      .$Post("api/store/verify_code?access_token=" + this.state.phone, { mobile: this.state.phone })
      .then((res) => {
        console.log(res);
      });
  };
  login = () => {
    axios
      .$Post("api/passport/login-by-token?access_token=" + this.state.phone, {
  ...
  setBtn = () => {
    if (this.state.time == 0) {
      return (
        <div className="btn" onClick={this.setTime}>
          发送
        </div>
      );
    } else {
  ...
  render() {
    return (
      <div className="login">
        ...
        <div className="chuchu">©上海上班族电子商务有限公司</div>
